...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === "POST" && segments[0] === "bulk") {
      ...
      for (let i = 0; i < sessions.length; i += 500) {
        ...
        for (const s of chunk) {
          if (!s.title || !s.date || !s.start || !s.end) { results.errors.push({ index: i + chunk.indexOf(s), error: "Missing required fields", session: s.title || "(no title)" }); continue; }
          const ref = s.session_id ? col.doc(s.session_id) : col.doc();
          batch.set(ref, { code: s.session_id || s.code || "", title: s.title, date: s.date, start: s.start || "", end: s.end || "", room: s.location || s.room || "", speakers: Array.isArray(s.speakers) ? s.speakers : [], format: s.format || "", recording: s.recording || "", sessionType: s.session_type || s.sessionType || "", mainTopic: s.topic || s.mainTopic || "", url: s.url || "", keyThemes: Array.isArray(s.key_themes) ? s.key_themes : [], attendees: [], createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() });
          results.created++;
        }
        await batch.commit();
      }
    ...
    if (segments.length === 1 && segments[0] !== "bulk") {
      ...
      if (req.method === "DELETE") { await ref.delete(); return res.json({ id: sessionId, deleted: true }); }
    }
