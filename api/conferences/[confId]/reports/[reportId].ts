...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === "POST") { await ref.update({ status: "published", publishedAt: FieldValue.serverTimestamp() }); return res.json({ reportId, status: "published" } satisfies ReportActionResponse); }
    if (req.method === "DELETE") { await ref.update({ status: "draft", publishedAt: null }); return res.json({ reportId, status: "draft" } satisfies ReportActionResponse); }
