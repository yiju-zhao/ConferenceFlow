...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const member = snap.data() as { status?: string };
    if (member.status !== "pending") return res.status(400).json({ error: "Member is not in pending status" });
    ...
    res.json({ userId, status: action === "approve" ? "approved" : "rejected" } satisfies MemberActionResponse);
  } catch (error) {
