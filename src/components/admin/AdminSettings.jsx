...
export default function AdminSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(t('admin.saved')); setTimeout(() => setMessage(""), 3000);
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="font-headline text-on-surface text-lg font-bold uppercase tracking-wider">{t('admin.conferenceSettings')}</h2>
      </div>
      <div className="bg-white border border-[#E8E4DF] rounded-lg p-6 max-w-2xl">
        <SettingsField label={t('admin.conferenceName')} field="name" placeholder={t('admin.conferenceName')} value={form.name} onChange={handleFieldChange} />
        <SettingsField label={t('admin.description')} field="description" type="textarea" placeholder={t('admin.description')} value={form.description} onChange={handleFieldChange} />
        <SettingsField label={t('admin.startDate')} field="startDate" type="date" value={form.startDate} onChange={handleFieldChange} />
        <SettingsField label={t('admin.endDate')} field="endDate" type="date" value={form.endDate} onChange={handleFieldChange} />
        <div className="mb-4">
          ...
          <div className="flex gap-3">
            {["public", "private"].map((v) => (
              <button key={v} onClick={() => setForm({ ...form, visibility: v })}
                className={`px-4 py-2 text-sm font-headline uppercase tracking-wider transition-all duration-150 rounded-lg ${form.visibility === v ? "bg-admin-teal text-white" : "bg-white border border-[#E8E4DF] text-secondary hover:border-admin-teal/30"}`}>{t(`admin.${v}`)}</button>
            ))}
          </div>
        </div>
        {form.visibility === "private" && (
          <div className="mb-4">
            <label className="block text-secondary text-xs uppercase tracking-wider mb-1">{t('admin.joinCode')}</label>
      ...
      {isSuperAdmin && (
        <div className="mt-10 max-w-2xl">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-headline text-red-600 text-lg font-bold uppercase tracking-wider">{t('admin.dangerZone')}</h2>
          </div>
